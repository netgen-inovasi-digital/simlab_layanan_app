...
class PembayaranUser extends BaseController
{
    ...
    private function aksiButton($id, $status, $file)
    {
        ...
        // Button "Upload Bukti" selalu aktif jika belum terverifikasi
        ...
        // Button "Proses/Kirim" hanya aktif jika sudah upload bukti dan belum terverifikasi
        $prosesDisabled = (empty($file) || $status == 2) ? 'disabled' : '';
        $prosesClass = (empty($file) || $status == 2) ? 'text-secondary' : 'text-success';
        $prosesTitle = empty($file)
            ? 'Upload bukti bayar terlebih dahulu'
            : ($status == 2 ? 'Sudah terverifikasi' : 'Kirim Bukti Pembayaran');
