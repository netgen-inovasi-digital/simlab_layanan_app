  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <!-- Kategori Status (status_layanan) -->
    ...
    sel.addEventListener('change', function () {
      ...
      if (table?.getConfig) {
        ...
        cfg.apiUrl = normalizeDoubleQuestion(
          buildApiUrlWithOptionalParam('<?= site_url("kajiulang/datalist") ?>', 'status_layanan', (val === '' ? null : val))
        );
  ...
  function loadDetail(id, kode_layanan) {
    ...
    if (kode_layanan) {
      ...
      if (cachedSampleData[kode_layanan]) {
        ...
        fetch(`<?php echo site_url("kajiulang/getSampleIdentity/") ?>${kode_layanan}`)
      ...
      // Jika kode_layanan tidak ada, jangan sembunyikan jika section sudah visible
  ...
  async function handleApproveReject(el, isAccept) {
    ...
    try {
      ...
      if (data.res) {
        ...
        const modalEl = document.getElementById('modalDetail');
        const savedLnKode = modalEl ? modalEl.dataset.kode_layanan : '';
