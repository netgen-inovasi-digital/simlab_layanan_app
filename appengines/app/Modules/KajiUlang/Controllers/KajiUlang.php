...
class KajiUlang extends BaseController
{
  ...
  public function datalist()
  {
    ...
    $lnStatusRaw = (string) ($this->request->getGet('status_layanan') ?? '');
    $statusArr = [];
    if ($lnStatusRaw !== '') {
    ...
    foreach ($list as $row) {
      ...
      if ((int) $row->status_layanan === 0 && empty($statusArr)) {
        continue;
      }
      ...
      $response[] = $this->formatStatusForManager($row->status_layanan, $row->kode_layanan, $user_id);

      $response[] = '<a href="javascript:void(0)" onclick="loadDetail(\'' . $id . '\', \'' . $row->kode_layanan . '\')" 
  ...
  public function saveKomentar()
  {
    ...
    try {
      $kode_layanan = $this->encrypter->decrypt(hex2bin($input['lnId']));
      $kode_layanan = (int) $kode_layanan;
    } catch (\Exception $e) {
      return $this->response->setJSON([
        'res' => false,
    ...
    // Check authorization
    if (!$this->kajiUlangModel->isUserAuthorizedForLayanan($kode_layanan, $user_id)) {
      return $this->response->setJSON([
        'res' => false,
    ...
    $ok = $this->kajiUlangModel->updateKomentarBatch($kode_layanan, $input['items']);
  ...
  private function formatStatusForManager($status_layanan, $kode_layanan, $userId)
  {
    // Check pending via r_tim
    $pendingCount = $this->kajiUlangModel->countPendingForManager($kode_layanan, $userId);
