...
class KajiUlangModel extends Model
{
    ...
    public function getLayananListForManager(array $lnKodeList, int $userId, array $statusArr = []): array
    {
        ...
        $builder->where('l.lnStatus !=', 2); // Exclude rejected
     ...
     * @return int
     */
    public function countLayananDetail($lnKode, int $ujiKode, ?int $statusLayanan = null): int
    {
        $builder = $this->db->table('t_layanan_detil');
     ...
     * @return int
     */
    public function countPendingForLayanan($lnKode): int
    {
        $builder = $this->db->table('t_layanan_detil');
        $builder->where('kode_layanan', $lnKode);
        $builder->groupStart()
     ...
     */
    public function approveLayananDetail($lnKode, int $ujiKode, int $managerId): int
    {
     ...
     * @return int Affected rows
     */
    public function rejectLayananDetail($lnKode, int $ujiKode, int $managerId): int
    {
        $result = $this->db->table('t_layanan_detil')
            ->where('kode_layanan', $lnKode)
            ->where('uji_kode', $ujiKode)
