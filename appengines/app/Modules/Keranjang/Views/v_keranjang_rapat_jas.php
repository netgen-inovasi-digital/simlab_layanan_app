        ...
        <div>
          <div class="d-flex justify-content-between align-items-center mb-3">
            ...
            <div class="d-flex align-items-center gap-2">
              ...
              <input type="file" class="form-control form-control-sm" id="fileSuratRapatJas" name="fileSuratRapatJas"
                accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" style="max-width: 300px;"
...
<script>
  ...
  function buildLayananUrl(jen = '') {
    const base = '<?= site_url("keranjang/rapatjas/dataListLayanan") ?>';
  ...
  document.getElementById('modalForm').addEventListener('shown.bs.modal', function () {
    ...
    previewKeranjangTable = createModal({
      apiUrl: '<?= site_url("keranjang/rapatjas/datalist") ?>',
  ...
  document.querySelector('#add').addEventListener('click', function () {
    fetch('<?php echo site_url("keranjang/rapatjas/checkVerified") ?>', {
      method: 'GET',
      headers: {
  ...
  function updateKeranjangCounter() {
    fetch('<?= site_url("keranjang/rapatjas/datalist") ?>')
      .then(res => res.json())
      .then(data => {
  ...
  if (!window.keranjangRapatJasClickHandlerAttached) {
    window.keranjangRapatJasClickHandlerAttached = true;
    ...
    document.addEventListener('click', function (e) {
      ...
      if (e.target.closest('.btnMasukkan')) {
        ...
        saveData({
          url: "<?= site_url('keranjang/rapatjas/submit') ?>",
          formData: formData,
  ...
  } // End of keranjangRapatJasClickHandlerAttached check
  ...
  function deleteItemFromPreview(eOrEl) {
    ...
    fetch("<?= site_url('keranjang/rapatjas/delete/') ?>" + idx)
      .then(res => res.json())
      .then(data => {
        if (data.xname && data.xhash) {
  ...
  (function populateJenFilterFallback() {
    ...
    if (select.options.length <= 1) {
      fetch('<?= site_url("keranjang/rapatjas/kategoriList") ?>', {
        method: 'GET',
        headers: {
          'Accept': 'application/json'
        }
