        ...
        <div class="card">
            ...
            <div class="card-header d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex align-items-center" style="gap:12px;">
                    <label class="card-title mb-0">Data Formulir Admin - Rapat JAS</label>
                </div>
...
<script>
    ...
    function createOrRefreshKerLayananTable(jen) {
        ...
        if (typeof ker_layananTable !== 'undefined' && ker_layananTable && typeof ker_layananTable.getConfig === 'function') {
            try {
                ...
                if (typeof ker_layananTable.fetchData === 'function') {
        ...
        if (ker_layananTable && typeof ker_layananTable.fetchData === 'function' && typeof ker_layananTable.getConfig === 'function') {
            ...
            ker_layananTable.fetchData = function (opts = {}) {
                try {
                    ...
                    console.warn('Normalization (ker_layananTable) failed:', err);
                }
                return orig(opts);
            };
        }
    ...
    function applyJenFilter() {
        try {
            ...
            console.warn('applyJenFilter error', e);
        }
    }
    ...
    var baseApiUrl = '<?php echo site_url("formuliradminrapatjas/datalist") ?>';
    if (kategoriFromUrl) {
    ...
    if (statusFilterEl) {
        statusFilterEl.addEventListener('change', function () {
            ...
            if (selectedValue === 'all') {
                tableConfig.apiUrl = '<?php echo site_url("formuliradminrapatjas/datalist") ?>';
            } else {
                tableConfig.apiUrl = buildApiUrlWithOptionalParam('<?php echo site_url("formuliradminrapatjas/datalist") ?>', 'kategoriLayanan', selectedValue);
            }
    ...
    function loadDetail(id) {
        const url = '<?php echo site_url("formuliradminrapatjas/detaillist/") ?>' + id;
    ...
    (function () {
        ...
        // Restore pelanggan dari session saat halaman dimuat
        async function restorePelangganFromSession() {
            try {
                const res = await fetch('<?= site_url("keranjangadmin/getPelanggan") ?>', {
                    method: 'GET',
                    headers: {
                        'X-Requested-With': 'XMLHttpRequest'
                    }
                ...
                if (data.res && data.pelanggan && data.pelanggan.user_id) {
                    // Set select value ke user_id yang tersimpan
                    sel.value = data.pelanggan.user_id;

                    // Trigger Select2 update jika menggunakan Select2
                    if (typeof $(sel).select2 === 'function') {
                        $(sel).val(data.pelanggan.user_id).trigger('change.select2');
                    }
                    ...
                    if (infoEl) {
                        infoEl.style.display = 'block';
                        infoEl.textContent = 'Untuk pelanggan: ' + (data.pelanggan.name || '-') + ' — ' +
                            (data.pelanggan.email || '-') + ' — ' + (data.pelanggan.status || '-');
                    }

                    // Update tombol checkout berdasarkan pelanggan + item count
                    const btnCheckout = document.getElementById('ker_btnCheckoutFromModal');
                    if (btnCheckout && data.items_count && data.items_count > 0) {
                        btnCheckout.disabled = false;
                    }
                }
            } catch (err) {
                console.warn('Restore pelanggan from session failed:', err);
            }
        }

        // Panggil saat halaman dimuat
        restorePelangganFromSession();
    })();
