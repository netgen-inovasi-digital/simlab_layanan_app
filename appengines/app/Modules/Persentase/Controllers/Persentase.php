...
class Persentase extends BaseController
{
	...
	public function dataList()
	{
		...
		$kdJenKode = $this->request->getGet('kdJenKode'); 

		if (!empty($kdJenKode)) {
			$model->where('kdJenKode', $kdJenKode);
		}

		$list = $model->orderBy('kdJenKode', 'ASC')->getAllData();

		$data = [];
		foreach ($list as $row) {
			$id = bin2hex($this->encrypter->encrypt($row->kdKode));
			$jenis = $modelJenis->getDataById('jenKode', $row->kdJenKode);

			$response = [];
			$response[] = '<span class="badge bg-secondary">' . esc($row->kdJenKode) . ' - ' . esc($jenis ? $jenis->jenNama : '') . '</span>';
			$response[] = $row->kdKolomLabel;
			$response[] = $row->kdPersenNONULM . ' %';
