...
{
    protected $table = 'simlab_t_layanan';
    protected $primaryKey = 'lnKode';
    protected $allowedFields = ['lnStatus', 'lnTgl', 'user_id', 'lnOrangNama', 'lnOrangJenis', 'lnOrangTipe'];
    ...
    /**
     * Update lnStatus di simlab_t_layanan
     * 
     * @param int $lnKode
     * @param int $status
     * @return bool
     */
    public function updateLayananStatus($lnKode, $status)
    {
        return $this->update($lnKode, ['lnStatus' => $status]);
    }
     ...
     * @param int $accUserId
     * @return array ['success' => bool, 'lnKode' => int|null, 'error' => string|null]
     */
    public function processDetailLhusTransaction($detKode, $newStatus, $accUserId)
    {
        ...
        try {
            ...
            if (!$detRow) {
                $db->transRollback();
                return ['success' => false, 'lnKode' => null, 'error' => 'Detil tidak ditemukan'];
            }
            ...
            $rowG = $this->getLayananStatusSummary($lnKode);

            $gTotal = (int) ($rowG['total'] ?? 0);
            $gCnt1 = (int) ($rowG['cnt1'] ?? 0);

            if ($gTotal > 0 && $gCnt1 === $gTotal) {
                $this->updateLayananStatus($lnKode, 6);

                // Update log sampel
                $currentTime = date('Y-m-d H:i:s');
                $this->updateLogSampelLhus($lnKode, $currentTime);
            }
            ...
            $db->transRollback();
            return ['success' => false, 'lnKode' => null, 'error' => $e->getMessage()];
        }
    }
