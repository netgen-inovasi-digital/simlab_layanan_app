<!-- Include Modal Tracking Lab -->
<?php require_once(__DIR__ . '/v_track_modal_lab.php'); ?>

<!-- Include Modal Keranjang Lab -->
<?php
echo view('Modules\Keranjang\Views\v_keranjang_lab', ['categories' => $categories ?? []]);
?>
  ...
  <div class="col-md-12">
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <button id="add_lab" class="btn btn-primary">
          <i class="bi bi-plus-circle-dotted"></i> Pesan Sewa Ruangan Lab Baru
        </button>
      </div>
      <div class="card-body">
        <table id="data-table" class="saytable border-top-bottom">
          <thead>
            <tr>
              ...
              <th show width="15%">File Pendukung</th>
            </tr>
          </thead>
          <tbody id="table-body">
          </tbody>
...
<script>
  ...
  var kategoriLabFromUrl = urlParams.get('kategoriLab');
  var katKodeFromUrl = urlParams.get('katKode');

  // Inisialisasi tabel utama menggunakan createTable
  var baseMainPath = '<?= site_url("pelayanan-lab/datalist") ?>';

  let initialParamKey = null;
  let initialParamValue = null;
  if (kategoriLabFromUrl && kategoriLabFromUrl !== '') {
    initialParamKey = 'kategoriLab';
    initialParamValue = kategoriLabFromUrl;
  ...
  function loadDetail(id) {
    const url = '<?php echo site_url("pelayanan-lab/detailList/") ?>' + id;
  ...
  function showTrackingModal(id, kode_layanan, status_layanan) {
    showFullTrackingModalLab(id, kode_layanan, status_layanan);
  }
  ...
  // Check verifikasi user saat halaman dimuat
  $(document).ready(function () {
    $.ajax({
      url: '<?= site_url('pelayanan-lab/checkVerified') ?>',
      method: 'GET',
      dataType: 'json',
      success: function (res) {
        if (!res.verified) {
          Swal.fire({
            ...
            text: res.msg || 'Harap hubungi admin untuk verifikasi akun.',
            confirmButtonText: 'OK'
          });
        }
      },
      error: function () {
        console.error('Gagal memeriksa status verifikasi');
      }
    });
  });
</script>