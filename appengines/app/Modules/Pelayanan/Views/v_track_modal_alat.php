...
<script>
  ...
  function showTrackingModal(id, kode_layanan, status_layanan) {
    ...
    // Mapping status_layanan -> step index untuk penyewaan alat
    const statusToStep = {
      1: 1,  // Review Petugas
    ...
    const targetStep = (statusToStep.hasOwnProperty(status_layanan) ? statusToStep[status_layanan] : 1);

    // Jika status termasuk reject, tandai step yang sesuai dengan kelas 'reject'
    if (rejectMap.hasOwnProperty(status_layanan)) {
      const r = document.querySelector(`#modalTrackingAlat .step[data-step="${rejectMap[status_layanan]}"]`);
      if (r) {
