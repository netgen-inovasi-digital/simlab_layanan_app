...
function createOrRefreshLayananTable(jen) {
    ...
    if (layananTable && typeof layananTable.getConfig === 'function') {
        try {
            ...
            try { if (typeof layananTable.destroy === 'function') layananTable.destroy(); } catch(e2){/*ignore*/ }
            layananTable = null;
        }
    }
    ...
    if (layananTable && typeof layananTable.fetchData === 'function' && typeof layananTable.getConfig === 'function') {
        ...
        layananTable.fetchData = function(opts = {}) {
            ...
            } catch (err) { console.warn('Normalization (layananTable) failed:', err); }
            return orig(opts);
        };
    }
}
...
document.getElementById('modalForm').addEventListener('shown.bs.modal', function () {
    ...
    if (previewKeranjangTable && typeof previewKeranjangTable.fetchData === 'function') {
        ...
        previewKeranjangTable.fetchData = function(opts = {}) {
            return origFetch(Object.assign({}, opts, { reload: true }));
        };
...
document.querySelector('#add').addEventListener('click', function () {
    fetch('<?php echo site_url("pelayanan/checkVerified") ?>', { method: 'GET', headers: { 'Accept': 'application/json' } })
    .then(r => r.json())
    .then(data => {
        if (data.verified) {
...
function calculateGrandTotal() {
    fetch('<?= site_url("pelayanan/keranjang/datalist") ?>')
                ...
                data.items.forEach(item => {
                    ...
                    for (let i = 0; i < item.length; i++) {
                        if (typeof item[i] === 'string' && item[i].indexOf('row-total') !== -1) {
                            const tmp = document.createElement('div'); tmp.innerHTML = item[i];
                            const rt = tmp.querySelector('.row-total');
                            if (rt) { totalStr = rt.textContent || rt.innerText || null; break; }
                        }
                    }
                    if (!totalStr) {
                        for (let i = item.length - 1; i >= 0; i--) {
                            if (typeof item[i] === 'string' && item[i].indexOf('Rp') !== -1) {
                                const tmp2 = document.createElement('div'); tmp2.innerHTML = item[i];
                                totalStr = (tmp2.textContent || tmp2.innerText || '').trim(); break;
                            }
                        }
                    }
                    if (totalStr) {
        ...
        .catch(err => { console.error('Error calculating grand total:', err); });
}
...
function doCheckout() {
    ...
    fetch('<?= site_url("pelayanan/keranjang/checkout") ?>', { method: 'POST', body: formData })
    .then(res => res.json())
    .then(data => {
        if (data.xname && data.xhash) {
            document.querySelectorAll('[name="' + data.xname + '"]').forEach(input => input.value = data.xhash);
...
(function populateJenFilterFallback(){
    ...
    if (select.options.length <= 1) {
        fetch('<?= site_url("pelayanan/keranjang/kategoriList") ?>', { method: 'GET', headers: { 'Accept': 'application/json' } })
        .then(r => r.json())
        .then(resp => {
