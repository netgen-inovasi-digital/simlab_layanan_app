    ...
    <div class="card">
      <div class="card-header d-flex justify-content-between align-items-center">
        ...
        <button id="add" class="btn btn-primary">
          <i class="bi bi-plus-circle-dotted"></i> Tambah Pengujian
        </button>
      </div>
      <div class="card-body">
    ...
    fetch('<?= site_url("pelayanan/lhulist/") ?>' + encId, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })
      ...
      .then(data => {
        ...
        const rows = items.map(row => {
          ...
          if (row.url && row.url !== '#') {
            buttons.push('<a class="btn btn-sm btn-outline-primary" href="' + escapeHtml(row.url) + '" target="_blank" rel="noopener"><i class="bi bi-eye"></i> Lihat</a>');
          }

          if (row.wa_url) {
            const adminLabel = row.admin_name ? 'Hubungi ' + escapeHtml(row.admin_name) : 'Hubungi Admin';
            buttons.push('<a class="btn btn-sm btn-outline-success ms-1" href="' + escapeHtml(row.wa_url) + '" target="_blank" rel="noopener"><i class="bi bi-whatsapp"></i> ' + adminLabel + '</a>');
          }

          const actionHtml = buttons.length > 0
            ? buttons.join(' ')
