...
class PelayananLab extends BaseController
{
    private $table = 'simlab_t_layanan';
    private $id = 'lnKode';
    protected $encrypter;
    private $sessionKey = 'keranjang_lab';
    ...
    public function index()
    {
        ...
        $data = [
            'title' => 'Data Pelayanan Sewa Ruangan Lab',
            'user' => $modelUser->getDataById('user_id', $user_id),
            'categories' => $categories ?? [],
        ];

        return view('Modules\Pelayanan\Views\v_pelayanan_lab', $data);
    }
    ...
    public function dataList()
    {
        ...
        // Query dengan JOIN ke t_layanan_detil untuk filter kode_jenis = 'C' (ruangan lab)
        $builder = $db->table($this->table . ' as t');
        $builder->select('t.lnKode, t.user_id, t.lnAccEmail, t.lnNoTransaksi, t.lnTgl, t.lnStatus, t.kuisioner, t.tgl_pelaksanaan, t.lhu_id');
        $builder->join('t_layanan_detil d', 'd.kode_layanan = t.lnKode', 'inner');
        $builder->where('t.user_id', $user_id);
        $builder->where('d.kode_jenis', 'C');  // Filter hanya ruangan lab (kode_jenis = 'C')
        $builder->groupBy('t.lnKode, t.user_id, t.lnAccEmail, t.lnNoTransaksi, t.lnTgl, t.lnStatus, t.kuisioner, t.tgl_pelaksanaan, t.lhu_id');
        ...
        // Siapkan map pembayaran terakhir per lnKode
        ...
        // Siapkan map file pendukung dari t_files_ruangan
        $fileRows = $db->table('t_files_ruangan')
            ->select('lnKode, files_pendukung')
            ->whereIn('lnKode', $lnKodes)
            ->get()->getResult();

        $fileMap = [];
        foreach ($fileRows as $f) {
            $ln = (int) $f->lnKode;
            $fileMap[$ln] = $f->files_pendukung ?? null;
        }
        ...
        foreach ($list as $row) {
            ...
            // Kolom 5: File Pendukung
            $filePendukung = isset($fileMap[$lnKodeInt]) ? $fileMap[$lnKodeInt] : null;

            if (!empty($filePendukung)) {
                $fileUrl = base_url('uploads/ruangan/' . $filePendukung);
                $response[] = '<a href="' . $fileUrl . '" target="_blank" class="btn btn-sm btn-outline-success">' .
                    '<i class="bi bi-file-earmark-text"></i> Lihat File' .
                    '</a>';
            } else {
                $response[] = '<span class="text-muted"><i class="bi bi-dash-circle"></i> Tidak ada file</span>';
            }
            ...
            return $this->response->setJSON([
                ...
                'data' => [
                    ...
                    'details' => array_map(function ($detailItem) {
                        ...
                        return [
                            'nama_ruangan' => $detailItem->nama_layanan ?? '-',
